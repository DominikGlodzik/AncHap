...
extern "C" {
  ...
  // id2 is the genotype
  int ibdMissGDist (int *n, int *id1, int *id2,  int *snpThreshold, double *genThreshold, double* snpGenMap, int* noSeqs, int* starts, int* ends,  int* chromends ) {
	...
	if ((((*id1==1)&&(*id2==3))||((*id1==3)&&(*id2==1)))&&(seqOn>0)) {
	  // opposing homozygote
	  if (((*snpGenMap-lastOppHGenLoc)>=*genThreshold)&&((pointer-lastOppSnpId)>*snpThreshold)) {
	  ...
	  seqOn = 0;
	  lastOppHGenLoc = *snpGenMap;
	  lastOppSnpId = pointer;
	}      
	else {
	...
	id1++; id2++;
	// move to the genetic location of the next SNP
	snpGenMap++;
      }
	...
	if (((*snpGenMap-lastOppHGenLoc)>=*genThreshold)&&((lastSnp-lastOppSnpId)>*snpThreshold)) {
